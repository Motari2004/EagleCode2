...
When generating app/page.tsx, the hero section MUST contain ALL 11 elements below in EXACT order.
...
<section className="relative h-screen flex items-center justify-center overflow-hidden">
  ...
  <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
    ...
    {/* ELEMENT 9: Description Paragraph */}
    <p className="text-base md:text-lg text-gray-300 mb-8 max-w-2xl mx-auto leading-relaxed">
      [2-3 sentence description explaining unique value proposition]
    </p>
    
    {/* ELEMENT 10: CTA Buttons (TWO) */}
    <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
      <Link 
        href="/[primary_link]" 
    ...
    {/* ELEMENT 11: Trust Indicators (3 minimum) */}
    <div className="flex flex-wrap gap-6 justify-center text-sm text-gray-300">
...
## HERO SECTION - RICH CONTENT REQUIREMENTS

The hero section MUST include ALL of these elements (not just h1 and p):

### REQUIRED ELEMENTS:

1. **Background Image** (/images/image_1.jpg) with dark overlay
2. **Badge/Pill** (e.g., "EST. 2024", "LIMITED EDITION", "NEW LAUNCH", "PREMIUM")
3. **Main Heading** (h1) - 2-5 words maximum
4. **Subheading/Tagline** - 1 sentence describing the brand/value
5. **Description paragraph** - 1-2 sentences explaining what you offer
6. **TWO Call-to-Action buttons**:
   - Primary: Gradient button, main action
   - Secondary: Outline button, alternative action
7. **Trust indicators** (optional) - Ratings, users count, guarantees
8. **Scroll indicator** - Animated down arrow
...
<section className="relative h-screen flex items-center justify-center overflow-hidden">
  ...
  <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
    ...
    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-500/20 backdrop-blur-sm border border-amber-500/30 mb-6">
      <Sparkles className="w-4 h-4 text-amber-400" />
      <span className="text-amber-400 text-sm font-medium">LIMITED EDITION</span>
    </div>
    ...
    {/* Subheading */}
    <p className="text-xl md:text-2xl text-amber-400 font-semibold mb-3">
      [Catchy tagline - 5-8 words]
    </p>
    
    {/* Description */}
    <p className="text-base md:text-lg text-gray-300 mb-8 max-w-2xl mx-auto leading-relaxed">
      [2-sentence description explaining what makes you unique and the value you provide]
    </p>
...
// ─────────────────────────────────────────────────────────────────────────────
// STEP 0 — FILE HEADER (ALWAYS FIRST LINE)
// ─────────────────────────────────────────────────────────────────────────────
// app/page.tsx MUST start with:
//   'use client';
// Because it uses: useState (FAQ accordion), onError (hero image fallback),
// onClick (CTA buttons). Missing 'use client' = Vercel build failure.
 
// ─────────────────────────────────────────────────────────────────────────────
// STEP 1 — REQUIRED IMPORTS
// ─────────────────────────────────────────────────────────────────────────────
// The AI MUST include ALL of these at the top of app/page.tsx:
//
//   'use client';
//   import { useState } from 'react';
//   import Link from 'next/link';
//   import { [ICON_A], [ICON_B], [ICON_C], [ICON_D], Plus, Minus } from 'lucide-react';
//
// Rules:
//   - Choose icons that match the PROJECT TYPE (see ADAPTIVE_ICON_MAP below)
//   - Plus and Minus are ALWAYS required for the FAQ accordion
//   - NEVER import icons that are not used in the file
//   - NEVER use @/ aliases — use relative paths only
 
const ADAPTIVE_ICON_MAP = {
  GYM:        ['Dumbbell', 'Zap', 'Trophy', 'Heart'],
  SCHOOL:     ['GraduationCap', 'BookOpen', 'Users', 'Star'],
  RESTAURANT: ['Utensils', 'Clock', 'MapPin', 'Star'],
  HOTEL:      ['Hotel', 'Wifi', 'Coffee', 'Shield'],
  ECOMMERCE:  ['ShoppingBag', 'Truck', 'ShieldCheck', 'Star'],
  PORTFOLIO:  ['Sparkles', 'Code', 'Globe', 'Layers'],
  COFFEE:     ['Coffee', 'Leaf', 'Package', 'Star'],
  TECH:       ['Cpu', 'Zap', 'Globe', 'Shield'],
} as const;
 
// ─────────────────────────────────────────────────────────────────────────────
// STEP 2 — DATA ARRAYS (DEFINED OUTSIDE THE COMPONENT)
// ─────────────────────────────────────────────────────────────────────────────
// All static data MUST be declared at the top level, NOT inside the function.
// This prevents re-creation on every render.
 
const DATA_RULES = {
  features: {
    count: 4,
    requiredFields: ['icon', 'title', 'desc', 'color'],
    uniquenessRule: 'Every title and desc MUST be different. No copy-paste.',
    example: [
      { icon: 'Truck',        title: 'Free Shipping',   desc: 'Free delivery on all orders over $50.',         color: 'from-blue-500 to-cyan-500'   },
      { icon: 'ShieldCheck',  title: 'Secure Payments', desc: '256-bit SSL encryption on every transaction.',  color: 'from-green-500 to-emerald-500'},
      { icon: 'Headphones',   title: '24/7 Support',    desc: 'Round-the-clock help from our dedicated team.', color: 'from-purple-500 to-pink-500'  },
      { icon: 'Star',         title: 'Premium Quality', desc: 'Handpicked products with a satisfaction guarantee.', color: 'from-yellow-500 to-orange-500'},
    ],
  },
 
  testimonials: {
    count: 3,
    requiredFields: ['quote', 'name', 'role', 'rating', 'initial'],
    uniquenessRule: 'Every quote, name, and role MUST be different.',
    example: [
      { quote: 'Absolutely exceeded my expectations. The quality is unmatched.', name: 'Sarah Johnson',   role: 'Verified Buyer',   rating: 5, initial: 'S' },
      { quote: 'Fast shipping, beautiful packaging, and a great product.',       name: 'Michael Chen',    role: 'Repeat Customer',  rating: 5, initial: 'M' },
      { quote: 'Impressive attention to detail. Will definitely order again.',   name: 'Emily Rodriguez', role: 'Happy Customer',   rating: 4, initial: 'E' },
    ],
  },
 
  stats: {
    count: 4,
    requiredFields: ['number', 'label', 'icon'],
    example: [
  ...
  faqs: {
    count: 4,
    requiredFields: ['q', 'a'],
    uniquenessRule: 'Questions and answers must be specific to the business type.',
    statePattern: "const [openIndex, setOpenIndex] = useState<number | null>(null);",
    togglePattern: "setOpenIndex(openIndex === idx ? null : idx)",
  },
} as const;
 
// ─────────────────────────────────────────────────────────────────────────────
// STEP 3 — SECTION TEMPLATES (EXACT JSX PATTERNS TO FOLLOW)
// ─────────────────────────────────────────────────────────────────────────────
...
const SECTION_TEMPLATES = {
  ...
  hero: `
    <section className="relative h-screen flex items-center justify-center overflow-hidden">
      {/* Background image — ONLY image allowed on home page */}
      <img
        src="/images/image_1.jpg"
        alt="Hero background"
        className="absolute inset-0 w-full h-full object-cover"
      ...
      {/* Content */}
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
        <span className="inline-block px-4 py-1 rounded-full bg-purple-500/20 text-purple-300 text-sm mb-6 backdrop-blur-sm">
          [EYEBROW_LABEL]
        </span>
        ...
        <p className="text-lg md:text-xl text-gray-200 mb-8 max-w-2xl mx-auto">
          [UNIQUE_TAGLINE — 10 to 15 words specific to this brand]
        </p>
        <div className="flex gap-4 justify-center flex-wrap">
          ...
          <Link
            href="/[SECONDARY_PAGE]"
            className="px-8 py-3 rounded-full border border-white/30 text-white font-semibold
      ...
      <div className="absolute top-20 left-10 w-72 h-72 bg-purple-500/20 rounded-full blur-3xl animate-pulse-slow pointer-events-none" />
      <div className="absolute bottom-20 right-10 w-96 h-96 bg-pink-500/20 rounded-full blur-3xl animate-pulse-slow pointer-events-none" style={{ animationDelay: '2s' }} />
    </section>
  `,
  ...
  features: `
    <section className="py-20 px-4 bg-gradient-to-br from-purple-950/20 via-transparent to-pink-950/20">
      <div className="container mx-auto">
        ...
        {/* Section heading — REQUIRED, must be visible */}
        <div className="text-center mb-12">
          <span className="text-purple-400 text-sm uppercase tracking-wider">Why Choose Us</span>
          ...
          <p className="text-gray-400 mt-4 max-w-2xl mx-auto">[SUBTITLE — specific to the brand]</p>
        </div>
          ...
          {features.map((feature, idx) => (
            <div
              ...
              className="group bg-white/5 backdrop-blur-sm border border-white/10
                         hover:border-purple-500/50 rounded-2xl p-6
        ...
        <div className="space-y-4">
          {faqs.map((faq, idx) => (
            ...
            >
              ...
              {openIndex === idx && (
                <div className="px-6 pb-5 text-gray-400 leading-relaxed text-sm border-t border-white/5 pt-3">
                  {faq.a}
                </div>
              )}
            </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// STEP 4 — COMPLETE COMPONENT SHELL
// ─────────────────────────────────────────────────────────────────────────────
// The AI MUST produce app/page.tsx in this exact shape.
// Replace [PLACEHOLDERS] with brand-specific content.
 
const COMPONENT_SHELL = `
'use client';
 
import { useState } from 'react';
import Link from 'next/link';
import { [ICON_A], [ICON_B], [ICON_C], [ICON_D], Star, Plus, Minus } from 'lucide-react';
 
// ── DATA — defined at top level, outside the component ──────────────────────
 
const features = [
  { icon: [ICON_A], title: '[UNIQUE TITLE 1]', desc: '[UNIQUE DESC 1]', color: 'from-blue-500 to-cyan-500'    },
  { icon: [ICON_B], title: '[UNIQUE TITLE 2]', desc: '[UNIQUE DESC 2]', color: 'from-green-500 to-emerald-500'},
  { icon: [ICON_C], title: '[UNIQUE TITLE 3]', desc: '[UNIQUE DESC 3]', color: 'from-purple-500 to-pink-500'  },
  { icon: [ICON_D], title: '[UNIQUE TITLE 4]', desc: '[UNIQUE DESC 4]', color: 'from-yellow-500 to-orange-500'},
];
 
const testimonials = [
  { quote: '[UNIQUE QUOTE 1]', name: '[NAME 1]', role: '[ROLE 1]', rating: 5, initial: '[A]' },
  { quote: '[UNIQUE QUOTE 2]', name: '[NAME 2]', role: '[ROLE 2]', rating: 5, initial: '[B]' },
  { quote: '[UNIQUE QUOTE 3]', name: '[NAME 3]', role: '[ROLE 3]', rating: 4, initial: '[C]' },
];
 
const stats = [
  { number: '[N1]', label: '[LABEL 1]', icon: '😊' },
  { number: '[N2]', label: '[LABEL 2]', icon: '🌍' },
  { number: '[N3]', label: '[LABEL 3]', icon: '📦' },
  { number: '[N4]', label: '[LABEL 4]', icon: '💬' },
];
 
const faqs = [
  { q: '[QUESTION 1 — specific to this business]', a: '[ANSWER 1]' },
...
// ─────────────────────────────────────────────────────────────────────────────
// STEP 5 — STYLING RULES (ENFORCED, NO EXCEPTIONS)
// ─────────────────────────────────────────────────────────────────────────────
 
const STYLING_RULES = {
  // Backgrounds
  body:        'bg-zinc-950 text-white antialiased',
  section_alt: 'bg-gradient-to-br from-purple-950/20 via-transparent to-pink-950/20',
  section_dark:'bg-gradient-to-r from-purple-950/50 to-pink-950/50',
  navbar:      'bg-gradient-to-r from-purple-950/80 via-zinc-950/80 to-pink-950/80 backdrop-blur-xl',
 
  // FORBIDDEN — will cause build rejection
  FORBIDDEN_BACKGROUNDS: [
    'bg-black',      // use bg-zinc-950 or gradient instead
    'bg-white',      // use bg-white/5 for glass cards
    'bg-gray-900',   // use gradient
    'bg-zinc-900',   // use gradient
  ],
 
  // Cards
  card_glass:    'bg-white/5 backdrop-blur-sm border border-white/10 hover:border-purple-500/50 rounded-2xl p-6 transition-all duration-300',
  card_gradient: 'bg-gradient-to-br from-purple-600/20 to-pink-600/20 border border-white/10 rounded-2xl p-6',
 
  // Text
  heading_gradient: 'bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent',
  heading_white:    'text-white font-bold',
  subtext:          'text-gray-400',
  eyebrow:          'text-purple-400 text-sm uppercase tracking-wider',
 
  // Buttons
  btn_primary:  'px-8 py-3 rounded-full bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold hover:opacity-90 hover:scale-105 transition-all duration-300 shadow-lg shadow-purple-500/25',
  btn_outline:  'px-8 py-3 rounded-full border border-white/30 text-white font-semibold hover:bg-white/10 transition-all duration-300',
 
  // Animations (all must exist in globals.css)
  required_animations: ['shimmer', 'float', 'pulse-slow', 'gradient'],
  gradient_text_class: 'gradient-text',  // defined in globals.css @layer components
} as const;
 
// ─────────────────────────────────────────────────────────────────────────────
// STEP 6 — PRE-OUTPUT CHECKLIST
// ─────────────────────────────────────────────────────────────────────────────
// The AI MUST verify ALL of these before writing app/page.tsx to the JSON output.
 
const PRE_OUTPUT_CHECKLIST = [
  "'use client' is line 1 — no blank lines or comments before it",
  "All imports are at the top — no dynamic imports mid-file",
  "features array has exactly 4 items, all with unique title and desc",
  "testimonials array has exactly 3 items, all with unique quote and name",
  "stats array has exactly 4 items",
  "faqs array has exactly 4 items, questions are business-specific",
  "All arrays are defined OUTSIDE the component function",
  "useState<number | null>(null) is declared inside the component",
  "Hero uses /images/image_1.jpg — no other pages use this image",
  "Hero onError uses optional chaining: parentElement?.classList",
  "Hero overlay is bg-black/50 (NOT a gradient overlay)",
  "Every section has a visible <h2> heading with gradient text classes",
  "Every section has an eyebrow <span> in text-purple-400 above the h2",
  "Feature cards use the card_glass style with hover:-translate-y-1",
  "Testimonial cards render star ratings using [...Array(5)].map()",
  "FAQ accordion uses openIndex state — NOT separate boolean per item",
  "No solid black or white backgrounds anywhere on the page",
  "No placeholder text: no 'Lorem ipsum', no 'Coming Soon', no 'TODO'",
  "No truncated arrays with '// ... more items' comments",
  "File ends with: export default function Home() — properly closed",
] as const;
 
// ─────────────────────────────────────────────────────────────────────────────
// STEP 7 — GLOBALS.CSS REQUIRED ENTRIES
// ─────────────────────────────────────────────────────────────────────────────
// These classes MUST exist in app/globals.css for the home page to render correctly.
 
const REQUIRED_CSS_CLASSES = `
/* Required in @layer components */
.gradient-text {
...
.animate-float      { animation: float 6s ease-in-out infinite; }
.animate-pulse-slow { animation: pulse-slow 3s ease-in-out infinite; }
.animate-gradient   { background-size: 200% auto; animation: shimmer 3s ease infinite; }
`;
 
// ─────────────────────────────────────────────────────────────────────────────
// EXPORTS — used by MASTER_BUILD_PROMPT parser
// ─────────────────────────────────────────────────────────────────────────────
...
REQUIRED STRUCTURE (11 elements minimum):
```tsx
<section className="relative h-screen flex items-center justify-center overflow-hidden">
  {/* Background Image */}
  <img 
  ...
  <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
    ...
    {/* Dual CTA Buttons */}
    <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
      <Link href="/[primary_link]" className="px-8 py-3.5 rounded-full bg-gradient-to-r from-amber-500 to-orange-600 text-white font-semibold hover:scale-105 transition-all duration-300 shadow-lg shadow-amber-500/25 inline-flex items-center gap-2 justify-center">
        Primary Action <ArrowRight className="w-4 h-4" />
      </Link>
      <Link href="/[secondary_link]" className="px-8 py-3.5 rounded-full border-2 border-white/30 text-white font-semibold hover:bg-white/10 hover:border-white/50 transition-all duration-300 inline-flex items-center gap-2 justify-center">
        Secondary Action
      </Link>
    </div>
    
    {/* Trust Indicators (3 minimum) */}
    <div className="flex flex-wrap gap-6 justify-center text-sm text-gray-300">
      <div className="flex items-center gap-2"><Star className="w-4 h-4 text-yellow-400 fill-yellow-400" /><span>4.9/5 from 2,000+ reviews</span></div>
      <div className="flex items-center gap-2"><Users className="w-4 h-4 text-cyan-400" /><span>50,000+ happy customers</span></div>
      <div className="flex items-center gap-2"><Shield className="w-4 h-4 text-green-400" /><span>30-day guarantee</span></div>
...
================================================================================
🚨 CRITICAL: HOME PAGE HERO BACKGROUND - ALWAYS USE IMAGE WITH RICH CONTENT 🚨
================================================================================
...
2. **STRUCTURE** - Use this EXACT pattern with ALL 12 elements:
...
<section className="relative h-screen flex items-center justify-center overflow-hidden">
  ...
  <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
    ...
    {/* ELEMENT 6: Badge/Pill */}
    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-500/20 backdrop-blur-sm border border-amber-500/30 mb-6 animate-pulse-slow">
      <Sparkles className="w-4 h-4 text-amber-400" />
      <span className="text-amber-400 text-sm font-medium uppercase tracking-wider">[UNIQUE_BADGE]</span>
    </div>
    ...
    {/* ELEMENT 9: Description Paragraph */}
    <p className="text-base md:text-lg text-gray-300 mb-8 max-w-2xl mx-auto leading-relaxed">
      [2-3 sentence description explaining unique value proposition. Include what makes you different and the main benefit for customers.]
    </p>
    
    {/* ELEMENT 10: Dual CTA Buttons */}
    <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
      <Link 
        href="/[primary-link]" 
        className="px-8 py-3.5 rounded-full bg-gradient-to-r from-amber-500 to-orange-600 text-white font-semibold hover:scale-105 transition-all duration-300 shadow-lg shadow-amber-500/25 inline-flex items-center gap-2 justify-center group"
      >
        Primary Action
