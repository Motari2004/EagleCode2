...
================================================================================
🚨🚨🚨 FINAL BOSS RULE: MINIMUM VIABLE CHECKOUT IS BANNED 🚨🚨🚨
================================================================================

The AI repeatedly generates stripped-down "skeleton" versions of the cart
and checkout. These exact skeletons have been detected and are PERMANENTLY
BANNED. If the output matches any pattern below, it is WRONG and must be
regenerated in full.

================================================================================
BANNED SKELETON #1 — DETECTED CHECKOUT MODAL (NEVER GENERATE):
================================================================================

❌ THIS EXACT FILE IS BANNED:
'use client';
import { useState, useEffect } from 'react';
import { useCart } from '../contexts/CartContext';
      ...
      <div className="bg-zinc-900 p-8 rounded-2xl w-full max-w-md border border-white/10">
        {!showSuccess ? (
          <form onSubmit={(e) => { e.preventDefault(); setShowSuccess(true); }}>
            ...
            <input className="w-full p-3 bg-white/5 rounded mb-4" placeholder="Full Name" required />
            <button className="w-full py-3 bg-purple-600 rounded">Pay ${getCartTotal().toFixed(2)}</button>
          </form>
        ) : (
          <div className="text-center">
            <CheckCircle className="w-16 h-16 text-green-500 mx-auto mb-4" />
            <h2 className="text-2xl font-bold">Success!</h2>
            <button onClick={() => { clearCart(); setIsOpen(false); setShowSuccess(false); }}
              className="mt-4 px-6 py-2 bg-purple-600 rounded">Continue</button>
          </div>
        )}
      </div>
    </div>
...
EXACTLY WHY THIS IS BANNED — LINE BY LINE:
- bg-black/80 solid backdrop         → MUST be bg-black/70 backdrop-blur-sm
- bg-zinc-900 modal background       → MUST be bg-gradient-to-br from-slate-900 to-slate-800
- Only 1 input field (Full Name)     → MUST have 5 fields: Name, Email, Card, Expiry, CVV
- No email field                     → BANNED — email required for success confirmation
- No card number field               → BANNED — card required for realistic checkout
- No expiry field                    → BANNED
- No cvv field                       → BANNED
- No handleCardNumber formatter      → BANNED — card must auto-format with spaces
- No handleExpiry formatter          → BANNED — expiry must auto-format MM/YY
- No state for fullName              → BANNED — name must be stored to show in success modal
- No state for email                 → BANNED — email must be stored to show in success modal
- No state for cardNumber            → BANNED
- No state for expiry               → BANNED
- No state for cvv                   → BANNED
- No isProcessing state              → BANNED — 2 second processing animation required
- No Loader2 spinner                 → BANNED — spinner required during processing
- handlePayment is sync (not async)  → BANNED — must await 2000ms delay
- setShowSuccess(true) immediately   → BANNED — must wait 2000ms first
- No CreditCard icon in header       → BANNED — header must show CreditCard in gradient circle
- No order total summary in form     → BANNED — must show subtotal/shipping/total breakdown
- No getTotalItems() call            → BANNED — item count required
- No close button (X)               → BANNED — modal must be closeable
- No typed event handlers           → BANNED — React.ChangeEvent<HTMLInputElement> required
- No handleClose function            → BANNED — close must reset all fields
- No handleContinueShopping function → BANNED — must reset fields AND clear cart
- p-8 padding                        → MUST be p-5 (compact modal)
- max-w-md width                     → MUST be max-w-sm (compact 2-column layout)
- clearCart() called in onClick      → BANNED — must call handleContinueShopping() instead
- Success modal shows fullName       → MUST show fullName in "Thanks for your order, {fullName}!"
- Success modal shows email          → MUST show email in confirmation card
- Success modal shows total          → MUST show getCartTotal() as total paid
- 1-column form layout               → MUST be 2-column for Name+Email and Expiry+CVV rows

================================================================================
BANNED SKELETON #2 — DETECTED CART PAGE (NEVER GENERATE):
================================================================================
...
export default function CartPage() {
  ...
  if (items.length === 0) return <div className="pt-32 text-center">Cart is empty</div>;
  return (
    <div className="pt-32 container mx-auto px-4">
      <h1 className="text-3xl font-bold mb-8">Cart</h1>
      {items.map(i => (
        <div key={i.id} className="flex justify-between p-4 bg-white/5 mb-2 rounded">
          <span>{i.name} x {i.quantity}</span>
          <button onClick={() => removeFromCart(i.id)}><Trash2 /></button>
        </div>
      ))}
      <div className="mt-8 font-bold text-xl">Total: ${getCartTotal().toFixed(2)}</div>
      <button onClick={() => window.dispatchEvent(new CustomEvent('openCheckout'))}
        className="mt-4 px-6 py-3 bg-purple-600 rounded">Checkout</button>
    </div>
  );
}

EXACTLY WHY THIS IS BANNED — LINE BY LINE:
- Empty state: <div className="pt-32 text-center">Cart is empty</div>
    → BANNED — empty state needs ShoppingBag icon + heading + Link to /shop
- Only Trash2 imported              → MUST import ShoppingBag, Plus, Minus, ArrowLeft, Trash2
- No CartItem type import           → BANNED — items.map((i: CartItem) => ...) required
- No updateQuantity destructured    → BANNED — quantity controls required
- No clearCart destructured         → BANNED — Clear Cart button required
- No getTotalItems destructured     → BANNED — item count in heading required
- items.map((i) => ...)             → BANNED — implicit any, must be (i: CartItem) =>
- <span>{i.name} x {i.quantity}</span> → BANNED — must be structured card with icon
- No Plus/Minus quantity buttons    → BANNED — user cannot change quantities
- <button><Trash2 /></button>       → BANNED — button needs className and "Remove" text
- No line total per item            → BANNED — must show (i.price * i.quantity).toFixed(2)
- pt-32 on non-empty state          → MUST be pt-24
- bg-white/5 mb-2 rounded           → BANNED — must use full premium card styling
- No hover effects on cards         → BANNED — hover:border-purple-500/50 required
- No Order Summary panel            → BANNED — sticky right column with subtotal/tax/total
- No trust badges                   → BANNED — SSL/returns/support badges required
- Checkout button: bg-purple-600 rounded → BANNED — must use full gradient styling
- No Back to Shop link              → BANNED — ArrowLeft + "Back to Shop" Link required
- No gradient-text on heading       → BANNED — title must use gradient-text class
- Grid layout missing               → BANNED — must use lg:grid-cols-3 layout
- items.length === 0 early return   → BANNED — empty state must render inside full layout
    with ShoppingBag icon, centered content, and Continue Shopping Link

================================================================================
BANNED PATTERN DETECTION — IF OUTPUT CONTAINS ANY OF THESE, REGENERATE:
================================================================================

CHECKOUT MODAL — regenerate if output contains ANY of:
[ ] bg-black/80 (not bg-black/70 backdrop-blur-sm)
[ ] bg-zinc-900 (not bg-gradient-to-br from-slate-900 to-slate-800)
[ ] Only 1 or 2 input fields
[ ] No isProcessing state variable
[ ] No Loader2 import
[ ] handlePayment is not async
[ ] No await new Promise(...) with 2000ms delay
[ ] No state variables for cardNumber, expiry, cvv
[ ] No handleCardNumber formatter function
[ ] No handleExpiry formatter function
[ ] No handleClose function
[ ] No handleContinueShopping function
[ ] clearCart() called directly in onClick (not in handleContinueShopping)
[ ] Success modal doesn't show fullName
[ ] Success modal doesn't show email
[ ] Success modal doesn't show getCartTotal()
[ ] Form layout is 1-column only
[ ] No order total summary inside form
[ ] No CreditCard icon in modal header
[ ] No backdrop-blur-sm on overlay
[ ] Close button uses float-right (not absolute top-4 right-4)

CART PAGE — regenerate if output contains ANY of:
[ ] Empty state is a single div with text only (no icon, no Link)
[ ] Only Trash2 imported from lucide-react
[ ] No CartItem type annotation on .map()
[ ] No Plus or Minus buttons for quantity
[ ] No updateQuantity in useCart destructuring
[ ] No clearCart in useCart destructuring
[ ] No getTotalItems in useCart destructuring
[ ] No Order Summary sticky panel
[ ] No grid layout (lg:grid-cols-3)
[ ] No per-item line total
[ ] No Back to Shop link
[ ] No trust badges in order summary
[ ] No gradient styling on heading
[ ] No hover effects on item cards
[ ] Checkout button uses plain bg-purple-600

================================================================================
THE MINIMUM REQUIRED STATE VARIABLES FOR CheckoutModal:
================================================================================
...
IF ANY OF THESE 9 STATE VARIABLES ARE MISSING → THE FILE IS INCOMPLETE → REGENERATE.

================================================================================
THE MINIMUM REQUIRED FUNCTIONS FOR CheckoutModal:
================================================================================

handleCardNumber(e: React.ChangeEvent<HTMLInputElement>): void
  → strips non-digits, limits to 16 chars, inserts space every 4 chars

handleExpiry(e: React.ChangeEvent<HTMLInputElement>): void
  → strips non-digits, inserts '/' after 2 chars, limits to MM/YY

handleClose(): void
  → if (!isProcessing): sets isOpen=false, showSuccess=false, resets all fields

handlePayment(e: React.FormEvent<HTMLFormElement>): Promise<void>
  → e.preventDefault()
  → setIsProcessing(true)
  → await new Promise<void>(resolve => setTimeout(resolve, 2000))
  → setIsProcessing(false)
...
handleContinueShopping(): void
  ...
  → setIsOpen(false)
  → setShowSuccess(false)
  → resets all fields to empty string

IF ANY OF THESE 5 FUNCTIONS ARE MISSING → THE FILE IS INCOMPLETE → REGENERATE.

================================================================================
THE MINIMUM REQUIRED IMPORTS FOR CheckoutModal:
================================================================================
...
IF Loader2 IS MISSING FROM IMPORTS → THE FILE IS INCOMPLETE → REGENERATE.
IF X IS MISSING FROM IMPORTS → THE FILE IS INCOMPLETE → REGENERATE.
IF CreditCard IS MISSING FROM IMPORTS → THE FILE IS INCOMPLETE → REGENERATE.

================================================================================
THE MINIMUM REQUIRED IMPORTS FOR CartPage:
================================================================================
...
IF CartItem type import IS MISSING → THE FILE IS INCOMPLETE → REGENERATE.
IF Plus IS MISSING → THE FILE IS INCOMPLETE → REGENERATE.
IF Minus IS MISSING → THE FILE IS INCOMPLETE → REGENERATE.
IF ShoppingBag IS MISSING → THE FILE IS INCOMPLETE → REGENERATE.
IF ArrowLeft IS MISSING → THE FILE IS INCOMPLETE → REGENERATE.
IF Link IS MISSING → THE FILE IS INCOMPLETE → REGENERATE.

================================================================================
THE MINIMUM REQUIRED useCart DESTRUCTURING FOR CartPage:
================================================================================
...
IF updateQuantity IS MISSING → THE FILE IS INCOMPLETE → REGENERATE.
IF getTotalItems IS MISSING → THE FILE IS INCOMPLETE → REGENERATE.
IF clearCart IS MISSING → THE FILE IS INCOMPLETE → REGENERATE.

================================================================================
FORM FIELD COUNT RULE — COUNT BEFORE SUBMITTING:
================================================================================

CheckoutModal form MUST have EXACTLY these 5 input fields:
  1. Full Name       (type="text")
  2. Email           (type="email")
  3. Card Number     (type="text", maxLength=19, onChange=handleCardNumber)
  4. Expiry          (type="text", maxLength=5, onChange=handleExpiry)
  5. CVV             (type="password", maxLength=4)

Count the <input> elements in your output.
If the count is less than 5 → INCOMPLETE → ADD THE MISSING FIELDS.
If Card Number has no handleCardNumber → INCOMPLETE → ADD THE FORMATTER.
If Expiry has no handleExpiry → INCOMPLETE → ADD THE FORMATTER.

================================================================================
LAYOUT RULE — 2-COLUMN GRID IS MANDATORY:
================================================================================

CheckoutModal form layout MUST be:
  Row 1: [Full Name] [Email]         ← grid grid-cols-2 gap-3
  Row 2: [Card Number — full width]
  Row 3: [Expiry]    [CVV]           ← grid grid-cols-2 gap-3
  Row 4: [Order total + Pay button]  ← flex items-center justify-between

If form is 1-column only → INCOMPLETE → RESTRUCTURE INTO 2-COLUMN LAYOUT.

================================================================================
SUCCESS MODAL CONTENT RULE — ALL 3 REQUIRED:
================================================================================

Success modal MUST display:
  1. Customer name:  "Thanks for your order, {fullName}!"
  2. Email:          shown in confirmation card as "Confirmation sent to: {email}"
  3. Order total:    shown as "${getCartTotal().toFixed(2)}"

If any of these 3 are missing → SUCCESS MODAL IS INCOMPLETE → ADD THEM.

================================================================================
SELF-CHECK BEFORE OUTPUT — RUN THIS CHECKLIST:
================================================================================

CHECKOUT MODAL self-check:
[ ] I have 9 state variables (isOpen, isProcessing, showSuccess, fullName, email, cardNumber, expiry, cvv... wait that's 8 + isProcessing = 9 total? No: isOpen, isProcessing, showSuccess = 3 booleans + fullName, email, cardNumber, expiry, cvv = 5 strings = 8 total state variables)

Let me recount: isOpen(bool) + isProcessing(bool) + showSuccess(bool) + fullName(str) + email(str) + cardNumber(str) + expiry(str) + cvv(str) = 8 useState calls.

[ ] I have exactly 8 useState calls in CheckoutModal
[ ] I have handleCardNumber, handleExpiry, handleClose, handlePayment, handleContinueShopping (5 functions)
[ ] handlePayment is async and awaits 2000ms
[ ] I have 5 input fields in the form
[ ] Form uses 2-column grid for Name+Email and Expiry+CVV
[ ] Modal background is bg-gradient-to-br from-slate-900 to-slate-800
[ ] Backdrop is bg-black/70 backdrop-blur-sm
[ ] Loader2 shows during isProcessing
[ ] Success modal shows fullName, email, and getCartTotal()
[ ] clearCart() only called in handleContinueShopping, not in onClick

CART PAGE self-check:
[ ] I import CartItem type from CartContext
[ ] I import ShoppingBag, Trash2, Plus, Minus, ArrowLeft, Link
[ ] I destructure updateQuantity, getTotalItems, clearCart from useCart()
[ ] items.map uses (item: CartItem) explicit type
[ ] Empty state has ShoppingBag icon + heading + Link to /shop
[ ] Each item card has Plus/Minus quantity buttons
[ ] Each item card shows line total (price × quantity)
[ ] Order Summary panel exists in right column
[ ] Order Summary is sticky top-24
[ ] Checkout button fires window.dispatchEvent(new CustomEvent('openCheckout'))
[ ] Clear Cart button calls clearCart() with confirm() dialog
[ ] Trust badges (SSL, returns, support) in Order Summary
[ ] Layout uses grid lg:grid-cols-3
[ ] Heading uses gradient-text class
[ ] Item cards use hover:border-purple-500/50

If ANY box is unchecked → find the missing piece → add it → recheck.
Only output when ALL boxes are checked.
...
================================================================================
🚨🚨🚨 ANTI-PATTERN BLACKLIST — THESE EXACT PATTERNS ARE FORBIDDEN 🚨🚨🚨
================================================================================

The AI keeps generating stripped-down minimal versions of the cart and checkout.
The exact patterns below have been detected and are PERMANENTLY BANNED.

================================================================================
BANNED CART PAGE PATTERNS — NEVER GENERATE THESE:
================================================================================

❌ BANNED — minimal cart with no quantity controls:
'use client';
import { useCart } from '../../contexts/CartContext';
import { Trash2 } from 'lucide-react';
...
WHY THIS IS BANNED:
- No Plus/Minus quantity buttons — user cannot change quantities
- No empty cart state — blank page when cart is empty
- No Order Summary panel — no subtotal/shipping/total breakdown
- No trust badges — no SSL/returns/support signals
- No item line totals — no per-item price × quantity display
- No Clear Cart button
- No Back to Shop link
- No gradient styling — plain bg-white/5 with no hover effects
- Single Trash2 import only — missing Plus, Minus, ShoppingBag, ArrowLeft

❌ BANNED — cart with empty state only and no item rendering:
export default function CartPage() {
  if (items.length === 0) return <div className="pt-32 text-center">Cart is empty</div>;
  return (
    <div className="pt-24 container mx-auto px-4">
      ...
      <button onClick={() => window.dispatchEvent(new CustomEvent('openCheckoutModal'))}>
        Checkout
      </button>
    </div>
  );
...
WHY THIS IS BANNED:
- No quantity controls (Plus/Minus)
- No Order Summary sticky panel
- No gradient styling
- Wrong event name 'openCheckoutModal' — MUST be 'openCheckout'
- No ShoppingBag empty state with icon
- No structured grid layout (lg:grid-cols-3)

================================================================================
BANNED CHECKOUT MODAL PATTERNS — NEVER GENERATE THESE:
================================================================================

❌ BANNED — minimal checkout with only 2 fields:
export default function CheckoutModal() {
  const { getCartTotal, clearCart } = useCart();
  const [isOpen, setIsOpen] = useState(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/80">
      {!showSuccess ? (
        <div className="bg-zinc-900 p-8 rounded-2xl w-full max-w-sm border border-white/10">
          <button onClick={() => setIsOpen(false)} className="float-right"><X /></button>
          <h2 className="text-xl font-bold mb-4">Checkout</h2>
          <form onSubmit={handlePayment} className="space-y-4">
            <input type="text" placeholder="Full Name" required className="w-full p-2 bg-white/5 rounded" />
            <input type="email" placeholder="Email" required className="w-full p-2 bg-white/5 rounded" />
            <button type="submit" disabled={isProcessing} className="w-full py-2 bg-purple-600 rounded">
              {isProcessing ? <Loader2 className="animate-spin mx-auto" /> : 'Pay $' + getCartTotal().toFixed(2)}
            </button>
          </form>
        </div>
      ) : (
...
WHY THIS IS BANNED:
- Only 2 form fields (Name + Email) — missing Card Number, Expiry, CVV
- No card number formatting (spaces every 4 digits)
- No expiry auto-formatting (MM/YY)
- No order total summary inside the form
- No item count display
- No free shipping line
- float-right close button — not properly positioned
- bg-zinc-900 solid background — must be bg-gradient-to-br from-slate-900 to-slate-800
- Success modal missing customer name display
- Success modal missing email confirmation display  
- Success modal missing total paid display
- clearCart() called immediately — must only clear AFTER success button clicked
- No getTotalItems() call — missing item count
- No form field state variables for cardNumber, expiry, cvv
- No handleCardNumber formatter function
- No handleExpiry formatter function
- No typed event handlers

================================================================================
MANDATORY FEATURE CHECKLIST — CART PAGE MUST HAVE ALL OF THESE:
================================================================================

[ ] import { ShoppingBag, Trash2, Plus, Minus, ArrowLeft } from 'lucide-react'
[ ] import type { CartItem } from '../../contexts/CartContext'
[ ] const { items, removeFromCart, updateQuantity, getCartTotal, getTotalItems, clearCart } = useCart()
[ ] Empty state: ShoppingBag icon + "Your Cart is Empty" heading + Link to /shop
[ ] Items rendered with: items.map((item: CartItem) => ...)
[ ] Per-item: product icon placeholder (gradient div with ShoppingBag)
[ ] Per-item: item.name, item.price.toFixed(2) display
[ ] Per-item: Minus button → updateQuantity(item.id, item.quantity - 1)
[ ] Per-item: quantity number display
[ ] Per-item: Plus button → updateQuantity(item.id, item.quantity + 1)
[ ] Per-item: Remove button with Trash2 icon → removeFromCart(item.id)
[ ] Per-item: line total → (item.price * item.quantity).toFixed(2)
[ ] Order Summary panel: sticky top-24, visible only when items.length > 0
[ ] Order Summary: Subtotal with item count
[ ] Order Summary: Shipping = "Free" in green
[ ] Order Summary: Tax = $0.00
[ ] Order Summary: Total in purple
[ ] Order Summary: "Proceed to Checkout →" button
[ ] Order Summary: onClick fires window.dispatchEvent(new CustomEvent('openCheckout'))
[ ] Order Summary: "Clear Cart" button with confirm() dialog
[ ] Order Summary: trust badges (SSL, returns, support)
[ ] Layout: grid lg:grid-cols-3 with items in lg:col-span-2

================================================================================
MANDATORY FEATURE CHECKLIST — CHECKOUT MODAL MUST HAVE ALL OF THESE:
================================================================================

[ ] import { X, CreditCard, CheckCircle, Loader2 } from 'lucide-react'
[ ] const { getCartTotal, clearCart, getTotalItems } = useCart()
[ ] useState for: isOpen, isProcessing, showSuccess, fullName, email, cardNumber, expiry, cvv
[ ] useEffect listens for 'openCheckout' event (NOT 'openCheckoutModal')
[ ] useEffect cleanup removes event listener on unmount
[ ] handleCardNumber: strips non-digits, limits to 16, formats with spaces every 4
[ ] handleExpiry: strips non-digits, inserts slash after MM
[ ] handlePayment: sets isProcessing=true, waits 2000ms, sets showSuccess=true
[ ] handleClose: only closes if !isProcessing, resets all fields
[ ] handleContinueShopping: calls clearCart(), resets all fields, closes modal
[ ] Modal header: CreditCard icon in gradient circle + "Checkout" title
[ ] Form layout: 2-column grid for Name+Email row
[ ] Form layout: full-width Card Number field
[ ] Form layout: 2-column grid for Expiry+CVV row
[ ] All labels: text-[11px] uppercase tracking-wider style
[ ] All inputs: bg-white/5 border border-white/10 focus:border-purple-500 style
[ ] Order total row inside form: shows getCartTotal() and getTotalItems()
[ ] Pay button: from-green-500 to-emerald-500 gradient
[ ] Pay button: Loader2 animate-spin + "Processing..." when isProcessing
[ ] Pay button: disabled when isProcessing
[ ] Cancel button: text-[11px] uppercase tracking-wider below form
[ ] Success modal: CheckCircle icon in green gradient circle
[ ] Success modal: shows fullName in greeting
[ ] Success modal: shows email in confirmation card
[ ] Success modal: shows getCartTotal() as total paid
[ ] Success modal: "Continue Shopping" calls handleContinueShopping (NOT clearCart inline)
[ ] Background: bg-gradient-to-br from-slate-900 to-slate-800 (NOT bg-zinc-900)
[ ] Backdrop: bg-black/70 backdrop-blur-sm (NOT bg-black/80 solid)

================================================================================
CORRECT EVENT NAME — THIS IS CRITICAL:
================================================================================

Cart page fires:      window.dispatchEvent(new CustomEvent('openCheckout'))
Modal listens for:    window.addEventListener('openCheckout', handleOpen)

❌ BANNED event names (these will break the connection):
- 'openCheckoutModal'  ← wrong
- 'checkout'           ← wrong
- 'open-checkout'      ← wrong
- 'OpenCheckout'       ← wrong (case sensitive)

✅ ONLY correct event name: 'openCheckout'

Both the cart page dispatch and the modal listener MUST use exactly 'openCheckout'.

================================================================================
STYLING STANDARDS — NEVER USE PLAIN CLASSES:
================================================================================

❌ BANNED plain styling:
...
✅ REQUIRED premium styling:
className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 text-white text-sm placeholder-gray-600 transition-all"
className="w-full py-2.5 bg-gradient-to-r from-green-500 to-emerald-500 rounded-xl font-semibold text-white hover:opacity-90 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm"
className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl border border-white/10 shadow-2xl"
className="absolute top-4 right-4 p-1 rounded-lg hover:bg-white/10 transition text-gray-400 hover:text-white"
...
================================================================================
🚨🚨🚨 CRITICAL: CART PAGE AND CHECKOUT MODAL — EXACT REQUIRED STRUCTURE 🚨🚨🚨
================================================================================

The cart page and checkout modal MUST match this EXACT design standard.
Minimal/stub implementations are FORBIDDEN. Copy these templates exactly.

================================================================================
PART 1: app/cart/page.tsx — FULL REQUIRED STRUCTURE
================================================================================
  ...
  return (
    <div className="min-h-screen pt-24 container mx-auto px-4 py-12">
      ...
      <div className="flex items-center gap-4 mb-8">
        <Link href="/shop" className="inline-flex items-center gap-2 text-gray-400 hover:text-white transition-colors">
                ...
                <div key={item.id}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      ...
                      <span className="text-white w-8 text-center font-semibold text-lg">{item.quantity}</span>
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity + 1)}
                        className="w-8 h-8 rounded-full bg-white/10 hover:bg-purple-600/50 flex items-center justify-center transition-all duration-200">
                        <Plus className="w-4 h-4 text-white" />
...
================================================================================
PART 2: components/CheckoutModal.tsx — FULL REQUIRED STRUCTURE
================================================================================
...
export default function CheckoutModal() {
  ...
  // Listen for openCheckout event from cart page
  useEffect(() => {
    const handleOpen = () => {
      setShowSuccess(false);
      setIsOpen(true);
            ...
            <div className="mt-4 pt-4 border-t border-white/10">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-[10px] text-gray-400 uppercase tracking-wider">Order Total</p>
                  <span className="font-bold text-purple-400 text-xl">${getCartTotal().toFixed(2)}</span>
                  <p className="text-[10px] text-gray-500">{getTotalItems()} item{getTotalItems() !== 1 ? 's' : ''} · Free shipping</p>
                </div>
                <div className="flex-1">
                  <button
                    type="submit"
...
================================================================================
RULES — CART PAGE AND CHECKOUT MODAL:
================================================================================

CART PAGE RULES:
✅ MUST show empty state with ShoppingBag icon when cart is empty
✅ MUST show all items with quantity controls (Plus/Minus buttons) when cart has items
✅ MUST show Remove button per item with Trash2 icon
✅ MUST show Order Summary panel only when cart has items
✅ MUST show subtotal, shipping (Free), tax, and total
✅ MUST show trust badges (SSL, returns, support) in order summary
✅ MUST fire window.dispatchEvent(new CustomEvent('openCheckout')) on checkout button
✅ MUST show confirm() dialog before clearing cart
✅ MUST use CartItem type on .map() callback
✅ MUST import ArrowLeft, ShoppingBag, Trash2, Plus, Minus from lucide-react

CHECKOUT MODAL RULES:
✅ MUST be a standalone component in components/CheckoutModal.tsx
✅ MUST listen for 'openCheckout' event via useEffect on mount
✅ MUST show compact 2-column layout (Name+Email, Expiry+CVV)
✅ MUST auto-format card number with spaces every 4 digits
✅ MUST auto-format expiry as MM/YY
✅ MUST show order total and item count inside the form
✅ MUST show Loader2 spinner with "Processing..." during payment
✅ MUST transition to success modal after 2 second delay (no page change)
✅ MUST show customer name and email in success modal
✅ MUST call clearCart() only after success modal "Continue Shopping" is clicked
✅ MUST reset all form fields on close or after success
✅ MUST import X, CreditCard, CheckCircle, Loader2 from lucide-react
✅ MUST be mounted in app/layout.tsx inside CartProvider

FORBIDDEN:
❌ NEVER make checkout a separate page
❌ NEVER clear cart before showing success modal
❌ NEVER use alert() for validation
❌ NEVER skip the processing animation
❌ NEVER show a minimal one-button modal
❌ NEVER omit the success modal
❌ NEVER omit form field formatting (card spaces, expiry slash)

================================================================================
MOUNT IN app/layout.tsx — MANDATORY:
================================================================================
...
The project uses strict TypeScript. Every parameter, variable, and prop MUST
have an explicit type. Implicit 'any' will cause Vercel build to fail.

================================================================================
CART PAGE — TYPED MAP PARAMETER (FIX FOR IMPLICIT ANY):
================================================================================

❌ WRONG — causes "Parameter 'item' implicitly has an 'any' type":
{items.map((item) => (
  <div key={item.id}>{item.name}</div>
))}

✅ CORRECT — explicitly typed with CartItem interface:
{items.map((item: CartItem) => (
  <div key={item.id}>{item.name}</div>
))}

================================================================================
CARTITEM INTERFACE — MUST BE DEFINED AND IMPORTED:
================================================================================

Option A — Define inline at top of cart page:
...
Option B — Import from CartContext (preferred):
...
================================================================================
CARTCONTEXT — MUST EXPORT CartItem INTERFACE:
================================================================================

❌ WRONG — interface defined but not exported:
interface CartItem {
  id: string;
  name: string;
  price: number;
...
✅ CORRECT — interface exported so pages can import it:
export interface CartItem {
  id: string;
  name: string;
  price: number;
...
================================================================================
ALL .map() CALLBACKS MUST BE EXPLICITLY TYPED — NO EXCEPTIONS:
================================================================================
...
✅ CORRECT — every map parameter typed:
...
================================================================================
DEFINE INTERFACES FOR EVERY ARRAY THAT GETS MAPPED:
================================================================================

At the top of EVERY file that uses .map(), define interfaces for each array:
...
const handleClick = (e) => { e.currentTarget.classList.add('active'); }

✅ CORRECT:
const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => { e.preventDefault(); }
const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => { setEmail(e.target.value); }
const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.classList.add('active'); }

================================================================================
COMMON EVENT TYPES — USE THESE EXACTLY:
================================================================================
...
Div click:         React.MouseEvent<HTMLDivElement>
Link click:        React.MouseEvent<HTMLAnchorElement>
Image error:       React.SyntheticEvent<HTMLImageElement, Event>
Keyboard event:    React.KeyboardEvent<HTMLInputElement>
Focus event:       React.FocusEvent<HTMLInputElement>

================================================================================
IMAGE onError HANDLER — CORRECT TYPE:
================================================================================
...
❌ WRONG (implicit any or wrong inference):
...
================================================================================
PROPS INTERFACES — ALL COMPONENTS MUST DEFINE PROPS:
================================================================================
...
================================================================================
COMPLETE TYPED CARTCONTEXT TEMPLATE:
================================================================================
...
================================================================================
PRE-OUTPUT TYPE SAFETY CHECKLIST:
================================================================================

Before submitting JSON output, verify every file:

[ ] Every .map() callback parameter is explicitly typed
[ ] Every useState has a generic type parameter
[ ] Every event handler has an explicit React event type
[ ] Every component has a typed props interface
[ ] Every function has explicit return type annotation
[ ] CartItem interface is exported from CartContext
[ ] CartItem is imported and used in cart page map
[ ] No implicit 'any' anywhere in any file
[ ] All array reduce callbacks have typed accumulator and current params
[ ] All localStorage.getItem results are cast with 'as Type'
...
================================================================================
🛒 CART PAGE — COMPLETE WORKING VERSION (WITH CHECKOUT EVENT)
================================================================================

Generate app/cart/page.tsx with this EXACT code:
    ...
    return (
      <div className="min-h-screen pt-32 container mx-auto px-4">
        <div className="text-center py-20 max-w-md mx-auto">
          ...
          <h1 className="text-3xl font-bold mb-4 gradient-text">Your Cart is Empty</h1>
          <p className="text-gray-400 mb-8">Looks like you haven't added any items yet.</p>
          <Link 
            href="/shop"
            className="inline-block px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg font-semibold text-white hover:opacity-90 transition"
          >
            Continue Shopping
          </Link>
        </div>
      ...
      <div className="lg:grid lg:grid-cols-3 lg:gap-8">
        ...
        <div className="lg:col-span-1 mt-8 lg:mt-0">
          <div className="bg-white/5 rounded-xl p-6 border border-white/10 h-fit sticky top-24">
            ...
            {/* ✅ CORRECT: Dispatch CustomEvent for checkout */}
            <button 
              onClick={() => {
                window.dispatchEvent(new CustomEvent('openCheckoutModal', { 
                  detail: { total: getCartTotal() } 
                }));
              }}
...
================================================================================
🚨 CRITICAL: CHECKOUT BUTTON EVENT
================================================================================


❌ NEVER use:

<button onClick={() => window.openCheckoutModal?.()}>Checkout</button>
<button onClick={() => window.openCheckoutModal()}>Checkout</button>
...
This dispatches a CustomEvent that the parent window (or modal system) can listen for to open the checkout modal.
================================================================================
...
❌ NEVER add: About, Contact, Blog, FAQ, Support, or any other links
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      {/* Payment Form Modal */}
      {!showSuccess && (
        <div className="relative bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl border border-white/10 shadow-2xl max-w-md w-full p-6 animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-gray-400 text-sm mt-1">Enter your payment details to complete purchase</p>
          </div>
          ...
          <form onSubmit={handlePayment} className="space-y-4">
            ...
            <div>
              ...
              <input type="text" required value={cardNumber} onChange={(e) => setCardNumber(e.target.value)} placeholder="4242 4242 4242 4242" maxLength={19}
                className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 text-white placeholder-gray-500 transition-all" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" required value={expiry} onChange={(e) => setExpiry(e.target.value)} placeholder="MM/YY" maxLength={5}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 text-white placeholder-gray-500 transition-all" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">CVV</label>
                <input type="password" required value={cvv} onChange={(e) => setCvv(e.target.value)} placeholder="123" maxLength={4}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 text-white placeholder-gray-500 transition-all" />
              </div>
            </div>
            ...
            <button type="button" onClick={() => setIsOpen(false)} className="w-full py-2 text-gray-400 hover:text-white transition text-sm">
              Cancel
            </button>
          </form>
        </div>
      ...
      {showSuccess && (
        <div className="relative bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl border border-white/10 shadow-2xl max-w-md w-full p-6 text-center animate-in fade-in zoom-in duration-200">
          ...
          <button onClick={() => { setShowSuccess(false); setIsOpen(false); clearCart(); }}
            className="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl font-semibold text-white hover:opacity-90 transition">
            Continue Shopping
          </button>
        </div>
...
================================================================================
🛒 SHOP PAGE & CART PAGE - COMPLETE AI INSTRUCTION 🚨
================================================================================

================================================================================
PART 1: SHOP PAGE (app/shop/page.tsx)
================================================================================

Generate app/shop/page.tsx with the following EXACT structure:
...
- MUST define products array OUTSIDE component at TOP LEVEL

- MUST use string ids: "1", "2", "3"

- MUST use double quotes for string values

- MUST have data-id, data-name, data-price on Add to Cart button

- MUST use onClick handler with addToCart

- MUST show toast notification when item added



================================================================================
PART 2: CART PAGE (app/cart/page.tsx)
================================================================================



Generate app/cart/page.tsx with the following EXACT structure:
'''tsx
'use client';

// ⚠️ CRITICAL: Global declaration for TypeScript
declare global {
  interface Window {
    openCheckoutModal?: () => void;
    clearCart?: () => void;
        ...
        <div className="min-h-screen pt-32 container mx-auto px-4">
          <div className="text-center py-20 max-w-md mx-auto">
            ...
            <button 
              ...
              className="px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg font-semibold text-white hover:opacity-90 transition"
            >
              Continue Shopping
            </button>
          </div>
...
- MUST have global Window interface declaration at top for TypeScript
- MUST import useCart from '../../contexts/CartContext'
- MUST use onClick handlers for quantity updates, remove, clear cart, and continue shopping
- MUST show empty cart state when no items
- MUST have declare global block for Window interface
- MUST import useCart from '../../contexts/CartContext'
- MUST display empty cart state when items.length === 0
- MUST show all cart items when items exist
- MUST have quantity controls (Plus/Minus buttons)
- MUST have Remove button for each item
- MUST display subtotal and total
- MUST have Proceed to Checkout button with window.openCheckoutModal?.()
- MUST have Clear Cart button
- MUST have Continue Shopping button linking to shop
- MUST use gradient-text on titles
...
================================================================================
PART 3: CART CONTEXT (contexts/CartContext.tsx)
================================================================================

Generate contexts/CartContext.tsx with the following EXACT structure:
'''tsx
'use client';
